...
const Composer: FC = () => {
  // Use a shared model config reference
  const configRef = (typeof window !== 'undefined') ? 
    (window as any).__MODEL_CONFIG_REF__ : { current: { modelName: "gpt-3.5-turbo", modelOwner: "openai" } };
  
  // Local state to control the UI
  const [modelConfig, setModelConfig] = useState({ 
    modelName: configRef?.current?.modelName || "gpt-3.5-turbo", 
  ...
  useEffect(() => {
    if (configRef) {
      ...
      console.log("Model config updated:", modelConfig);
    }
  }, [modelConfig]);
